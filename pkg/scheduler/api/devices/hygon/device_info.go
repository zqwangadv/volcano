...
func (ds *DCUDevices) AddQueueResource(pod *v1.Pod) map[string]float64 {
	...
	ids, ok := pod.Annotations[AllocatedIDsToAllocateAnnotations]
	if !ok {
		klog.Errorf("pod %s has no annotation hami.io/devices-to-allocate", pod.Name)
		return res
	}
...
func (ds *DCUDevices) addResource(annotations map[string]string, pod *v1.Pod) {
	ids, ok := annotations[AllocatedIDsToAllocateAnnotations]
	if !ok {
		klog.Errorf("pod %s has no annotation hami.io/dcu-devices-to-allocate", pod.Name)
		return
	}
	podDev := decodePodDevices(ids)
	for _, val := range podDev {
...
func (ds *DCUDevices) Allocate(kubeClient kubernetes.Interface, pod *v1.Pod) error {
	if HygonHAMiDCUEnable {
		...
		annotations[AllocatedIDsToAllocateAnnotations] = podDevicesAnnotations
